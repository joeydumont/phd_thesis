...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{figs/IntensityHistory_inkscape.pdf}
  \caption{Intensity of laser systems as a function of time (approximative).
...
We first take the Fourier transform of the fields, i.e.
  ...
  \end{equation}
and then take the curl of both equations.\todo{How much of the derivation do I
...
\todo[inline]{Insert figure of domain decomposition.}
...
\todo[inline]{[Maybe move this ahead.]}

Discuss paraxial approximation in the incident field case (exp(-ikz) phase, mostly),
and the fact that all frequency components have the same waist size (I recall an article
mentioning a rationale for that).

Generic bases for arbitrary field models (linear + radial polarization).

\subsection{Parallelization and Performance Analysis}

Parallel efficiency, data output benchmarks (setting the stage for other
similar procedures.)

\subsection{Numerical Verifications}

Convergence with respect to the number of points on the mirror.
Verify that the numerically computed fields solve the Maxwell equations.
Verify that the numerically computed fields conserve energy.

\section{Comparisons with Other Field Models}

\subsection{Analytical Field Models}

Salamin field model, other ST implementations?

\subsection{The Richards-Wolf Formalism}

Statement of the RW formalism. Comment on derivation from RW to ST.
Comparison of results with parabolic mirror in the frequency domain.

\section{Properties of Tightly Focused Fields}

Visualization of the field and comments on important properties.
Max of the field in time domain, and also in frequency components.
3D visualization of the tranverse part of the beam.
3D visualization of the longitudinal components of the beam.
Importance of the longitudinal component as a function of the focal length
and its possible effects on SF-QED observables (FORESHADOWING).

\subsection{Longitudinal Components}

Relative strength of the longitudinal component as a function of the
focal length.

\subsection{Gouy Phase}

Gouy phase of tightly focused fields (deviation from Gaussian Gouy phase).
Mention analytical solution for linear polarization.
Computation of phase from 2D Fourier transforms.
...
SC mostly used to model the fine details of tightly focused fields in an

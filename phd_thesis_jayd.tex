...
\documentclass[11pt,SymmetricalJury,PhD]{inrsthesis/inrsthesis}
\usepackage{lipsum}
% ----------------------------------------------------------------- %
% --                          Packages                           -- %
% ----------------------------------------------------------------- %
...
\usepackage[unicode=true,
      ...
      pdftitle={Strong-field quantum electrodynamics in tightly focused fields},
      bookmarks=true,
...
\todo[inline]{https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.121.053001}
...
\todo[inline]{https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.121.043202
https://arxiv.org/abs/1807.09271}
...
and $\vb{B}$ fields \cite{DiPiazza2012}. \todo{Cite physical phenomena that
display strong fields, such as astrophysical phenomena, and heavy ion
collisions. Mention HPLs last, and praise their controllability, reproducibility
and full diagnostics. Conclude to reaching high intensities.} Reaching higher
...
% \todo{Add figures of the fields separately, then show the overlap integral
%   for each. Comment on it.}
% Even with added terms, \ldots\todo{Same thing again.}
...
fields.
\todo[inline]{Argue why this is important, cite paragraph+lit.}
...
% In addition to these ``physical'' requirements on the theoretical framework, we
% also require that the resulting software implementation satisfies some
% properties. As this software project forms the basis for our future work on the
% computation of strong-field QED observables, we must make sure that the code is
% modular enough to accommodate the demands of each target observable. We also
% require that the resulting framework that a software implementation can run in a
% reasonable amount of time on a computing cluster.
% \todo{Fix large space in itemize.}
...
\todo[inline]{https://journals.aps.org/pra/abstract/10.1103/PhysRevA.98.012134}
...
Dirac equation. It is very difficult to solve this system self-consistently
\cite{https://arxiv.org/abs/1607.04224}. In the EH framework, there are no
fermions, and we are left with Maxwell's equations and a source term related to
the 4-point interaction. This forms the basis of the method developed in Chapter
\ref{chapter:fwm} aimed at computing FWM yields in tightly focused fields.
...
\begin{description}
  ...
  the results of this thesis. In section \ref{sec:methods.paraxial}, we discuss
  the paraxial optics framework, its use in the description of beam transport
  and other areas, and the limits of its basic approximation. We show that for
  \textit{tightly focused fields}, the paraxial framework breaks down and new
  models are needed. In the following sections, we discuss analytical solutions
  of Maxwell's equations that are valid in the tight focusing regime, and
  diffraction integral formulations of the same equations.

  In section \ref{sec:methods.vector-diffration}, we discuss the Richards-Wolf
  and Stratton-Chu diffraction integrals. We show some qualitative discrepancies
  in their predictions in a specific parameter sector. While we conclude that
  the correct model should be determined experimentally, we settle for the
  Stratton-Chu integrals for the reminder of the thesis based on mathematical
  controllability of the latter's assumptions. The numerical evaluation of the
  Stratton-Chu diffraction integrals constitutes the central part of this
  thesis, and underpins most of our research contributions. We review the
  mathematical derivation of the diffraction integrals, and discuss some
  specific features of the integrals.

  \item [Chapter \ref{chapter:stratton-chu} -- The Stratton-Chu Diffraction Integrals]
  In this chapter, we discuss our parallel implementation of a computer code
  that evaluates the Stratton-Chu diffraction integrals for generic incident
  ...
  our main research contribution. The design and features of the MPI-based code
  is thoroughly presented Section \ref{sec:sc.parallel-implementation}.

  Then, in Section \ref{sec:sc.case-study}, we study the properties of a short,
  linearly polarized Gaussian beam after focusing through a parabolic mirror. We
  show that, in this geometry, the focused beam has an elliptical spot shape,
  and that there exists a focal length value for which the longitudinal
  components dominate over the transverse ones, in the focal region.
  ...
  \textit{in vacuo}, a strong-field QED process. In Section
  \ref{sec:fwm.background}, we review the Euler-Heisenberg Lagrangian and the
  derivation of a non-linear modification to Maxwell's equations from it. We
  also e detail the development of a physics module to the StrattoCalculator
  meant to evaluate FWM \textit{in vacuo} in arbitrary electromagnetic fields.

  In Section \ref{sec:fwm.case-study}, we study FWM \textit{in vacuo} in
  different geometries. We start from typical parabolic mirror geometries, such
  as the off-axis mirror, to less common geometries such as on-axis mirrors, and
  parabolic mirrors where the focal point is inside the aperture of the mirror.
  We show that there is parametric maximum in the number of FWM photons when
  varying the focal length of such mirrors.
...
of modeling tightly focused fields: (1) paraxial field models, (2) analytical
solutions of Maxwell's equations in vacuum, specifically the Salamin solutions
...
reflection off the focusing optics. This leads us to (3): diffraction integral
\todo{Rephrase the list.}
frameworks, the Richards-Wolf and Stratton-Chu formalisms. By comparing fields
from different models, we show that the Stratton-Chu formalisms shows features
unique to the tight focusing regime that the analytical models fail to show. We
also discuss a discrepancy between the Richards-Wolf and Stratton-Chu
...
\label{sec:methods.paraxial}

The starting point is, as usual, the Maxwell equations. Their macroscopic
version, in Lorentz-Heaviside units with $c=1$, reads
  \begin{subequations}
  \begin{align}
    \nabla\times\vb{E} + \frac{\partial\vb{B}}{\partial t}  &= 0 \\
...
is an electric current distribution.We will mostly be interested in the
electromagnetic field distribution in the $V_\infty$ region, i.e. in vacuum. In
this case, we have $\vb{D}=\vb{E}$ and $\vb{H}=\vb{B}$. In this section, we will
also take vanishing sources $\vb{J}_e\equiv0$.
...
radially polarized beams \cite{Salamin2015a,Salamin2015b}.

\todo[inline]{Add paragraph on Lax substitution to solve for both transverse
and longitudinal components separately.}
...
equation in a Taylor series in $\epsilon$. This expansion takes the form
  \begin{equation}
    \psi = \sum_{m=0}^\infty \psi^{(2m)}
  \end{equation}
where $\psi^0$ is a solution of the paraxial equation, and $\psi^{(2m)}$ obey
\cite{Salamin2007}
  \begin{equation}
  \begin{aligned}
    \left(\nabla_\perp^2-4i\partial_z\right)\psi^{(0)}  &=0, \\
    \left(\nabla_\perp^2-4i\partial_z\right)\psi^{(2m)} &= -\epsilon^2\partial_z^2\psi^{(2m-2)}.
  \end{aligned}
  \end{equation}


Solutions to these equations can be incredibly useful when the fields are mildly
focused. Indeed, down to beam waists of $w_0=\lambda$,
$\epsilon\sim\nicefrac{1}{3}$, we can expect the series to converge given enough
terms \cite{Borghi2003}. However, as we will show later, we can produce fields
with waists of $w_0\sim0.4\lambda$ in a single optical element configuration,
and we expect to see features that this series cannot replicate.

\todo[inline]{Cite this: https://arxiv.org/abs/1807.04482}

\todo[inline]{Short discussion of propagation methods because of first-order in $z$.}
\todo[inline]{Failure of the approximation for large divergences (tight focusing), but maybe
other propagation methods? (cite literature).}

\section{Closed-Form Solutions of Maxwell Equations in Vacuum}

\todo[inline]{Short introduction on vacuum solutions, then mention specific
for tight focusing (e-dipole, Salamin). CSBs?}


\section{Vector Diffraction Integrals}
\label{sec:methods.vector-diffration}
...
\todo[inline]{Fix the multiply defined $\alpha$ issue.}

\subsection{Derivation of the Stratton-Chu Diffraction Integrals}

The strategy to derive diffraction integrals from Maxwell's equations is to
...
consequence on the modeling of reflection off conducting surfaces.
...
Our starting is a slightly modified version of Maxwell's equations
  \begin{subequations}
  \begin{align}
  ...
  \end{align}
  \end{subequations}
where $\vb{J}_m$ is a fictitious magnetic current distribution that generates an
incident electric field \cite{Schelkunoff1936,Elliott2003}. This approach allows
us to postulate the existence of equivalent sources on a given surface that
reproduce a given incident field model outside the volume enclosed by that
surface.
...
this dissertation, as vacuum is non-dispersive, i.e. $\omega=k$. We thus have
  \begin{subequations}
  \begin{align}
    \nabla\times\vb{E} - ik\vb{B} &= \vb{J}_m, \\
    \nabla\times\vb{B} + ik\vb{E} &= \vb{J}_e.
  \end{align}
  \end{subequations}
...
that expresses the field at any point in $V_\infty$ as integrals of the (yet
unknown) fields $\{\vb{E},\vb{B}\}$ over the surface of the scatterer. Hence, to
be able to actually compute the values of the fields at any point in $V_\infty$,
it is first necessary to know the values of the fields on $S$ as a function of
the incident fields. Taking the limit $\vb{r}'\rightarrow S$ results in an
integral equation for the fields on $S$, which can be solved iteratively.
However, care must be taken in the evaluation of this limit as $g$ has a simple
pole at $\vb{r}=\vb{r}'$, i.e. on the surface of the mirror. This singularity is
dealt with analytically by deforming the surface at $\vb{r}=\vb{r}'$ to a
hemispherical surface with vanishing radius $R$. Let this surface be
$S_\epsilon$. The surface integrals can now be written as
  \begin{align}
...
field categories. See Table \ref{tab:methods.comparison.parameters.linear} for
...
\begin{table}
  ...
  \caption[Parameters of the linearly polarized, Gaussian incident beam.]
          {Parameters of the linearly polarized, Gaussian incident beam used in
  ...
  \label{tab:methods.comparison.parameters.linear}
\end{table}

\subsection{Lax Models}

In this section, we discuss some models based on the Lax series \cite{Lax1975},
where the solution of the wave equation is expanded in a power series in
$\epsilon=w_0/z_R$. The fact that the electric field, magnetic field and vector
potential all obey the same differential equation, and that the technique above
calls for a choice of boundary condition allows some flexibility in modeling
moderately focused fields with the Lax series. Below, we will study two such
choices and show that they fail to reproduce some qualitative and quantitative
features of tightly focused fields.

Our first model, described in \cite{Borghi2003}, describes a linearly polarized
electric field with the boundary condition
  \begin{equation}
    \vb{E}_\perp(\vb{r}_\perp,z) = \psi_0(\vb{r}_\perp)\vu{x}.
  \end{equation}
The transverse and longitudinal fields are thus computed via
  \begin{align}
    \psi^{(2m)}(\vb{r}_\perp,z) &= \left(\frac{i}{2k}\right)^m
          \sum_{p=1}^m c_p^{(m)}z^p\partial_z^{m+p}\psi^{(0)}(\vb{r}_\perp,z) \\
    \phi^{(2m+1)}(\vb{r}_\perp,z) &= \frac{i}{k}\left[\partial_x\psi^{(2m)}+\partial_z\phi^{(2m-1)}\right]
  \end{align}
where $\phi^{-1}=0$ and
  \begin{equation}
    c_p^{(m)} = \frac{(2m)!}{m(p-1)!(m-1)!(m+p)!}
  \end{equation}

For comparison's sake, we choose the typical Gaussian beam solution
as our boundary condition, i.e.
  \begin{equation}
    \psi_0(r,z) = \frac{w_0}{w(z)}\exp\left[-\frac{r^2}{w(z)^2}-ikz-ik\frac{r^2}{2R(z)}+i\arctan\left(\frac{z}{z_R}\right)\right]
  \end{equation}
where
  \begin{align}
    ...
    z_R  &= \frac{kw_0^2}{2}.
  \end{align}
To evaluate the non-paraxial corrections to this field model, we evaluate
the derivatives symbolically using the SymPy Python library \cite{Meurer2017}.
\todo{Fix \v{S} problem in bibliography.}

In the tight focusing regime with $w_0=\lambda/2$ and a truncation order of
$m=4$, we see that 

\begin{figure}
  \centering
  \includegraphics{figs/LaxSeries-4.pdf}
  \caption{Electromagnetic fields predicted by the Lax series for a linearly
           polarized beam with $w_0=\lambda/2$, truncation order $m=4$ and
           $\lambda=800\,\si{\nano\metre}$.}
  \label{fig:methods.lax-series4}
\end{figure}

%This formal approach justifies the inherently \textit{ad hoc}
%assumption of the usual paraxial theory of simply dropping the second-order $z$
%derivative in favour of a first-order equation.


Add short section on direct Lax method. Compare higher-orders. Weird behaviour
for magnetic field, qualitatively very different.

Add paragraph on Salamin's approach. Show that $E_x$ and $B_y$ seem to be elliptical,
but no in phase (one's perfectly circular and the other elliptical depending
on the parameters chosen).

Add paragraph on CSPSW, which is qualitatively closer to the vectorial approaches,
but unfortunately diverges in the region of interest.


\begin{figure}
  \centering
  \includegraphics{figs/Salamin.pdf}
  \caption{Electromagnetic fields predicted by the Salamin version of the Lax
           series for a linearly polarized beam with $w_0=\lambda/2$, truncation
           order $m=2$ and $\lambda=800\,\si{\nano\metre}$.}
  \label{fig:methods.salamin}
\end{figure}

\begin{figure}
  \centering
  \includegraphics{figs/CSPSW.pdf}
  \caption{Electromagnetic fields predicted by convergence of two spherical waves
           with $w_0=\lambda/2$.}
  \label{fig:methods.cspsw}
\end{figure}
...
\todo[inline]{Add figures of both fields in time at $t=0$ and $t=\lambda/4$.}
...
\begin{table}
  \begin{tabular}{l|p{3cm}}
                  & $E_y=B_x$, anti-symmetric elliptical $E_x$/$B_y$, strong longitudinal components \\
  Lax series      & \faCheck \\
  Salamin (Lax)   & \faClose \\
  CSPSW           & --\\
  Salamin (Tight) & \\
  ...
  Stratton-Chu    &
  \end{tabular}
  \caption{Qualitative features that different models predict.}
\end{table}


\begin{table}
  \begin{tabular}{l|p{3cm}}
                  & Arbitrary spatial dependence, power spectrum and spectral phase, aberrations, space-time couplings, arbitrary focusing geometries, \\
  Lax series      & \\
  Salamin (Lax)   & \\
  CSPSW           & \\
  Salamin (Tight) & \\
  ...
  Stratton-Chu    &
  \end{tabular}
  \caption{Effects that each model can implement.}
\end{table}

\todo[inline]{Another method: https://www.osapublishing.org/josaa/abstract.cfm?uri=josaa-35-8-1356}
...
\label{sec:sc.parallel-implementation}
...
\todo[inline]{List of software requirements: Modular. In next section we wish to compute
an integral over a predefined region of space for each \textbf{frequency} of the beam.
In other section we wish to compute the field at discrete points of space
and \textbf{time}. This is possible via modularity.}

\todo[inline]{Discuss unit testing.}

\todo[inline]{Flowchart: Define the number of processors --> Define incident beam (spectral shape+total energy, spatial dependence) --> Define geometry of the mirror (S) --> Define region for integration. --> Start the calculation
in Fourier space --> (Optionnally) Computee the field in time. --> Output the fields.}
...
\todo[inline]{Link this discussion with the other methods discussion elsewhere.}

\todo[inline]{Find place for this comment: In the implementation, we provide two
modes of operation. Either we compute the field at a user-specified point, or on
a predefined mesh of points suitable for integration for later use.}
...
efficiently. \todo{Citation.} In the second case, we expect the code to scale
...
\todo[inline]{Add sources for the data.}
...
\todo[inline]{Connection to: https://www.osapublishing.org/josaa/abstract.cfm?uri=josaa-35-6-985}
...
$\alpha$.
  ...
  \begin{figure}
    \centering
    %\includegraphics{/opt/data/thesis-data/Fields/Stratto/na_vsf_lin_g/Ex_ellipticity.pdf}
...
\subsection{Aberrations in the Focal Plane}

\todo[inline]{Fragility of the longitudinal components.}
...
\todo[inline]{Cite conclusion of https://journals.aps.org/prx/abstract/10.1103/PhysRevX.8.031004.}
...
\todo[inline]{Intersting feasibility argument:
https://arxiv.org/abs/1807.00101}

\subsection{Parallel Implementation}

\section{Four-Wave Mixing With Tightly Focused Fields}
\label{sec:fwm.case-study}
...
masking the incident field to create artificial shadows in the far-field where
...
% \chapter{Radiation of Charged Particles in Tightly Focused Fields}
% \label{chapter:electron_trajectories}

% \section{The Lorentz Force Equation and Radiation Reaction}

% \section{Generic Implementation}

% \section{Measuring the Intensity of High-Power Lasers}

% \section{Radiation Fields as a Possible Source of Noise}
...
%\todo[inline]{Quantum limit to coherent emission by accelerating particles:
%https://journals.aps.org/prl/abstract/10.1103/PhysRevLett.121.010402.}
...
Bessel functions. Maybe compare to RW at that point. Then introduce the
power-series representation, and the incomplete gamma function expressions}
...
can be incredibly difficult to solve. \todo{Cite B. King (shock QED).} To simplify the process, we linearize these

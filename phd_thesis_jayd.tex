...
At this point, we can see that the propagation phase, $e^{-ikz'}$, factors
out of the integrand. The remaining term, $e^{2ikz't^2}$ can be interpreted
as the phase incurred by the focusing process.
...
theorem
  \begin{equation}
    (1-t^2)^n = \sum_{k=0}^n (-1)^k t^{2k},
  \end{equation}
such that we can use
  \begin{equation}
    \int t^{2k+1}e^{2izt^2}dt = -\frac{1}{2}\frac{\Gamma(k,-izt^2)}{(-iz)^k} +C
  \end{equation}

...
\todo[inline]{Derivation of the paraxial wave equation. Show that both
$\vb{E}$, $\vb{B}$ and $\vb{A}$ obey the same equation. Use that to
define linear and radially polarized solutions.}

\todo[inline]{Discuss paraxial approximation in the incident field case (exp(-ikz) phase, mostly),
...
\todo[inline]{Parallel efficiency, data output benchmarks (setting the stage for other
similar procedures).}

\subsection{Numerical Verifications}

\todo[inline]{Convergence with respect to the number of points on the mirror.}
\todo[inline]{Verify that the numerically computed fields solve the Maxwell equations.}
\todo[inline]{Verify that the numerically computed fields conserve energy.}
...
\todo[inline]{Visualization of the field and comments on important properties.}
\todo[inline]{Max of the field in time domain, and also in frequency components.}
...
\todo[inline]{and its possible effects on SF-QED observables (FORESHADOWING).}

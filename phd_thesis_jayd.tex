...
In this thesis, we are primarily interested in developing numerical tools for the
...
this thesis, as vacuum is non-dispersive. We thus have
  \begin{subequations}
  \begin{align}
...
compact support, the integral over the sources get limited to the volume $V_e+V_m$.
...
Let us now go back to Eq.~\eqref{eq:sc.field-stratton-chu-sancer-hypersingular}
and solve them.
To properly model the material properties of the scatterer, we impose the
appropriate boundary conditions. In this thesis, we take the boundary conditions
...
Given our goal of modeling broadband pulses
In this section, we discuss the evaluation of Eqs­.~\eqref{eq:sc.stratton-chu-poa-og}
in parallel. The implementation is based on OpenMPI, a C software library that
provides facilities to pass messages between processes as a means of executing
tasks in parallel.
...
Flowchart: Define the number of processors --> Define incident beam (spectral shape+total energy, spatial dependence) --> Define geometry of the mirror (S) --> Define region for integration. --> Start the calculation

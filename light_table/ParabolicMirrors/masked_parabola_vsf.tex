...
\begin{tikzpicture}[scale=3]
  ...
  \draw[domain=0:\zmax,samples=1000,ultra thick,smooth,name path=parabola1] plot (\x,{sqrt(4*\focal*(\x+\focal))});
  \draw[domain=0:\zmax,samples=1000,ultra thick,smooth,name path=parabola2] plot (\x,{-sqrt(4*\focal*(\x+\focal))});
  ...
  \foreach \x in {-0.85,-0.75,-0.65,...,-\rmin}
  {
    \parabola{\x}\draw (\zmaxplot,\x) -- (\myvalue,\x) coordinate (F) -- (0,0) coordinate (A);
    \pgfextractangle{\angle}{A}{F}
    \draw (0,0) -- ++(\angle+180:0.25cm);
  }

  % -- This draws the light rays on parabola2.
  \foreach \x in {0.85,0.55,0.45,...,\rmin}
  {
    \parabola{\x}\draw (\zmaxplot,\x) -- (\myvalue,\x) coordinate (F) -- (0,0) coordinate (A);
    \pgfextractangle{\angle}{A}{F}
    \draw(0,0) -- ++(\angle+180.0:0.25cm);
  }

  % -- Prepares the endpoints for the shadow.
  \pgfmathsetmacro{\rmaxsha}{0.80}
  ...
  % -- Draw the mask.
  \draw[fill=black!50] (1.5, \rminsha) rectangle (1.55, \rmaxsha);
  \draw                (\zmaxplot,0.65) -- (1.55, 0.65);
  \draw                (\zmaxplot,0.75) -- (1.55, 0.75);
  ...
  \coordinate[] (S1)  at ($(0,0)!-0.75cm!(A2)$);
  \coordinate[] (S2)  at ($(0,0)!-0.75cm!(A3)$);
